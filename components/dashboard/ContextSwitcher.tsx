...
export default function ContextSwitcher() {
  ...
  return (
    <>
      ...
      {toast && (
        <div className="fixed bottom-6 left-6 z-50 flex items-center gap-2.5 px-4 py-3 rounded-xl border backdrop-blur-md bg-surface-overlay/90 border-brand-500/20 text-brand-400 font-mono text-xs shadow-[0_8px_32px_rgba(0,0,0,0.5)]">
          <span className="relative flex h-2 w-2 shrink-0">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-brand-500 opacity-75"></span>
            <span className="relative inline-flex rounded-full h-2 w-2 bg-brand-500 shrink-0"></span>
          </span>
          <span>[System Sync]: Switched to {toast}</span>
        </div>
      )}
    </>
  );
